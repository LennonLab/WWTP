...
#Set up directory and load packages
```{r, results = 'hide', warning=FALSE, message=FALSE}
rm(list=ls())

package.list <- c('knitr','png', 'vegan', 'ggplot2', 'tidyverse', 'here', 'ggpmisc', 'ggpubr', 'cowplot', 'picante', 'AICcmodavg') 
...
otu <- read.csv(here("data", "wwtp.otu.csv"), sep = ",", header=TRUE)

# Metadata
meta <- read.table(here("data","wwtp.metadata.txt"), sep="\t", header=TRUE)

richness <- read.table(here("data","wwtp.richness.csv"), sep = ",", header = TRUE)
...
meta$Richness <- NA
for(x in richness$X){
  meta$Richness[meta$Sample_ID == x] <- richness$S[richness$X == x]
  }
...
summary(lm(Richness~I(abs(Latitude))+I(Latitude^2), data=meta))
...
summary(lm(Richness~I(abs(Latitude))+I(abs(Latitude)^2), data=meta))
```
